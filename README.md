...
- [x] **Rate Limiting:** Protects endpoints against brute-force attacks.
- [x] **Gzip Compression:** Minimizes network payload size.
- [x] **Indexed Queries:** MongoDB indexes on `email`, `category`, and `createdAt` for fast search operations.
- [x] **Server-side Pagination:** Prevents client-side memory bloat by loading data in configurable chunks (8/page).
- [x] **Input Sanitization:** Normalized emails and validation checks on every mutation.
- [x] **Zero AI Watermarks:** Clean, enterprise-style architecture and production UI.
